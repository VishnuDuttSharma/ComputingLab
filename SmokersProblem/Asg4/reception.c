 ...
 *
 *	Function forks, creates child and runs various processes in them
 */
...
void print_guestbook()
{
   ...
   printf("Name\t\tIn \tOut \tRoom No.\n");
   printf("\t\tTime\tTime\n");
   pFile = fopen ("GuestBook.txt" , "r");
   if (pFile == NULL)
   {
...
int main(int argc, char *argv[])
{
    ...
    while(1){
        ...
        printf("1. Print Guest Book \n2. Print Service Room Occupancy Table \n3. Exit \n" );
        scanf("%d", &resp);
        if (resp == 1)
            print_guestbook();
        else if (resp == 2)
