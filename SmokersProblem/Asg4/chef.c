 ...
 *  @brief \var Array for storing current value of semaphore
 */
...
void initChef()
{
  ...
  maachSem = sem_open( SEM_NAME_MAACH, O_CREAT, 0644, 0);
  if( maachSem == SEM_FAILED)
    perror("sem_open");
  dalSem = sem_open( SEM_NAME_DAL, O_CREAT, 0644, 0);
  if( dalSem == SEM_FAILED)
    perror("sem_open");
  bhaatSem = sem_open( SEM_NAME_BHAAT, O_CREAT, 0644, 0);
