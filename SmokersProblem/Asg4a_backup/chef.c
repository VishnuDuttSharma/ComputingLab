...
int shmid, shmItem, shmCount, shmMaach, shmDal, shmBhaat;; //Shared memory
int *chefAddr;
int *itemCnt;
/**
...
int main(int argc, char *argv[])
{
  ...
  //sem_getvalue(&maachSem, &maachVal);
  //sem_getvalue(&dalSem, &dalVal);
  //sem_getvalue(&bhaatSem, &bhaatVal);

  printf("Total:\n \tMaach : %d \n \tDal: %d \n \tBhaat: %d \n", itemCnt[0], itemCnt[1], itemCnt[2]);
  //printf(" Maach : %u \n Dal: %u \n Bhaat: %u \n PID: %d\n", maachVal, dalVal, bhaatVal, getpid());
