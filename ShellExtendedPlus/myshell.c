...
/**
\fn char *inputTaker(char *commandLine)

@param commandLine Pointer to the command string
 
@return Pointer to the final command string
...
void runMyshell()  
{
  ...
  int stdoBack, stdiBack; // Back

 file descriptor for stdout and stdin
  ...
  { 
    ...
    while(1)
    {
      ...
      if( strcmp(command[0], "mycd") == 0 ) 
        ...
        { 
          sprintf(cmdPath, "%s/mymv.out", cwd);
          runCommand(cmdPath, argLen);  
        } 
